...
stacktrace_fast(void **buffer, int size, int skip)
{
...
libgcc_backtrace_callback(struct _Unwind_Context *ctx, void *_data)
{
	struct libgcc_backtrace_data *data =
		(struct libgcc_backtrace_data *)_data;
	if (data->skip > 0) {
		data->skip--;
	...
	*(data->pos++) = ip;
	return (ip && data->pos < data->end) ? _URC_NO_REASON :
					       _URC_END_OF_STACK;
}

__attribute__((always_inline)) static inline int
stacktrace_slow(void **buffer, int size, int skip)
{
	struct libgcc_backtrace_data data;
	data.skip = skip;
