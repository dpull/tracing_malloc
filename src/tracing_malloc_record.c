...
static void _backup_proc_maps()
{
	...
	while (1) {
		int bytes_read_len = fread(file_name_buffer, 1,
					   sizeof(file_name_buffer), src);
		if (bytes_read_len <= 0)
			break;

		int bytes_write_len =
			fwrite(file_name_buffer, bytes_read_len, 1, dst);
...
_record_alloc(int add_flag, void *ptr, size_t size)
{
	struct hashmap_value *hashmap_value;
	void *buffer[sizeof(hashmap_value->address) /
			     sizeof(hashmap_value->address[0]) +
		     STACK_TRACE_SKIP];
	memset(buffer, 0, sizeof(buffer));
	int count = stack_backtrace(buffer, sizeof(buffer) / sizeof(buffer[0]),
				    STACK_TRACE_SKIP);
	...
	memcpy(hashmap_value->address, buffer + STACK_TRACE_SKIP,
	       sizeof(hashmap_value->address));
	return 0;
}
