...
static inline intptr_t round_up(intptr_t value, intptr_t alignment) { return (value + alignment - 1) & ~(alignment - 1); }
...
static int is_internal_malloc(void* ptr) { return (char*)ptr >= internal_malloc_buffer && (char*)ptr < internal_malloc_buffer_end; }
...
__attribute__((destructor)) static void uninit(void) { record_uninit(); }

void* sys_malloc(size_t size) { return g_sys_malloc(size); }

void sys_free(void* ptr) { g_sys_free(ptr); }
