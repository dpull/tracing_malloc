...
{
	_Static_assert(sizeof(int64_t) == sizeof(void *),
...
struct hashmap *hashmap_create(const char *file, size_t value_max_count)
{
	...
	struct hashmap_value *hashmap_value =
		(struct hashmap_value *)filemapping_create_readwrite(file,
								     length);
	if (!hashmap_value)
		return NULL;
	...
	struct hashmap *hashmap =
		(struct hashmap *)sys_malloc(sizeof(*hashmap));
